...
{
    //网络上加载数据
    NSString *url = [NSString stringWithFormat:@"%@?catalog=4&pageIndex=%d&pageSize=%d", api_post_list, 1, 20];
...
- (void)footerRefreshing
{
    ...
    pageIndex ++;
    NSString *url = [NSString stringWithFormat:@"%@?catalog=4&pageIndex=%d&pageSize=%d", api_post_list,pageIndex, 20];
    [[AFOSCClient sharedClient] getPath:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        //XML数据解析
        GDataXMLDocument *document = [[GDataXMLDocument alloc] initWithData:operation.responseData options:0 error:nil];
        NSString *xpath = @"/oschina/posts/post";
