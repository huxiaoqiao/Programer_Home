...
@implementation TweetTopScrollView

@synthesize nameArray;
@synthesize scrollViewSelectedChannelID;

+ (TweetTopScrollView *)shareInstance {
    static TweetTopScrollView *_instance;
...
- (void)selectNameButton:(UIButton *)sender
{
    ...
    if (!sender.selected) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            [shadowImageView setFrame:CGRectMake(sender.frame.origin.x, 0, [[_buttonWithArray objectAtIndex:BUTTONID] floatValue], 30)];
            
        } completion:^(BOOL finished) {
            if (finished) {
                //设置新闻页出现
                [[TweetRootScrollView shareInstance] setContentOffset:CGPointMake(BUTTONID*320, 0) animated:YES];
                //赋值滑动列表选择频道ID
...
- (void)setButtonSelect
{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        ...
        [shadowImageView setFrame:CGRectMake(button.frame.origin.x, 0, [[_buttonWithArray objectAtIndex:button.tag-100] floatValue], 30)];
